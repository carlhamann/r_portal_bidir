...
struct Portal {
    ...
    Ray sample_ray(Rng& rng) const {
        // Sample position
        const auto p_portal = p1 + e1 * rng.u() + e2 * rng.u();

        // Sample direction
        const auto wo_local = math::sample_cosine_weighted(Vec2(rng.u(), rng.u()));
        const auto [u, v] = math::orthonormal_basis(n);
        const Mat3 to_world(u, v, n);
    ...
    // Evaluate PDF for ray sampling
    Float pdf_ray(Vec3 wo) const {
        if (glm::dot(wo, n) < 0_f) {
            return 0_f;
        }
        const auto pA = invA;
